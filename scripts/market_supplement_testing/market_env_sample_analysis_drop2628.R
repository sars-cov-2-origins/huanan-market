...
## drop all samples from street 4, stalls 26/28
stall_to_remove = list('A14','A15','A90','A88','A87')
pos_sites = filter(pos_sites, !(title %in% stall_to_remove))
...
same_stall=list(list('Q61','Q64','Q69','Q70','Q68'),list('A2','A18','A20'),list('F98','F100'),list('Q93','A61'))
...
for (i in seq(1, to=length(same_stall), by=1)){ # skips street 4, stall 26,28
...
h0 = 13.91397 # use bandwidth from cross validation
pdf('relative_risk_pos_vs_all_drop2628.pdf')
...
pal <- colorRampPalette(cols)
pdf('kde_all_stalls_drop2628.pdf')
...
cols = c("#FFFFFF",cols)
pal <- colorRampPalette(cols)
pdf('kde_positives_sample_level_drop2628.pdf')
