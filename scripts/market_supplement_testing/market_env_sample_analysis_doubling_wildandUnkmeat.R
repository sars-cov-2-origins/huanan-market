...
## read in positive sample locations, project to 3857
wild_sites = gdf[gdf$group=='WildlifeVendor',]
wild_sites$title = lapply(wild_sites$title, FUN=function (x) strsplit(x," (",fixed=T)[[1]][1]) #strip info about sample type
wild_sites = st_transform(wild_sites, 3857)

unkmeat_sites = gdf[gdf$group=='UnknownMeat',]
unkmeat_sites$title = lapply(unkmeat_sites$title, FUN=function (x) strsplit(x," (",fixed=T)[[1]][1]) #strip info about sample type
unkmeat_sites = st_transform(unkmeat_sites, 3857)
...
# calculate minimum distance of all sites to a wildlife vendor
for (i in seq(1, to=dim(all_sites)[1], by=1)){
  if ((min(as.numeric(st_distance(all_sites$geometry[i],wild_sites$geometry)))<1.) || (min(as.numeric(st_distance(all_sites$geometry[i],unkmeat_sites$geometry)))<1.)){
    all_sites = rbind(all_sites,all_sites[i,])
    # print(all_sites$title[i])
  }
}
...
control_ppp = ppp(x = all_coords[,1]+rnorm(dim(all_coords)[1])*eps,
                  y = all_coords[,2]+rnorm(dim(all_coords)[1])*eps,
...
h0 = 13.79936 # use bandwidth from cross-validation
pdf('relative_risk_pos_vs_all_doublewUnkMeat.pdf')
...
pal <- colorRampPalette(cols)
pdf('kde_all_stalls_doublewUnkMeat.pdf')
...
cols = c("#FFFFFF",cols)
pal <- colorRampPalette(cols)
pdf('kde_positives_sample_level_doublewUnkMeat.pdf')
