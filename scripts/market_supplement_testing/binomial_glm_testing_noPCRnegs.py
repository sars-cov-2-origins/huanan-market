...
same_stall=[['A14','A15','A90','A88','A87'],['Q64','Q69','Q68'],['A2','A18','A20'],['F98','F100'],['G93','A61']]
...
df_pos = df_pos[(~df_pos.index.str.contains('Q37')) & (~df_pos.index.str.contains('Q61')) & (~df_pos.index.str.contains('Q70'))]
df_all = pd.concat((df_pos,df_neg),axis=0)

#option for only considering live mammal vendors
wildlife_stands = gdf[(gdf['group']=='WildlifeVendor')]
...
for N in range(5, 11):
    print(f'Case with {N} samples per stall')
    ...
    coefs = list(robjects.r('coef(summary(model))[,1]')[1:])
...
df_pvals.T.to_csv('pvals_noPCRnegs.csv')