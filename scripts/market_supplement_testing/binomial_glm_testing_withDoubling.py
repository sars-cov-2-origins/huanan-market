...
for N in range(5, 11):
    ...
    df_all.loc[df_all['wild_dist']<1,'fails'] = N*2 - df_all.loc[df_all['wild_dist']<1,'count']
    ...
    robjects.globalenv['model'] = robjects.r("glm('cbind(count, fails)~wild_dist+case_dist', family = binomial, data = rdf)")
    ...
    #extract p values for wildlife distance, case distance
...
df_pvals.to_csv('pvals_doubling.csv')
