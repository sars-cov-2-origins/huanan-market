...
for (i in seq(1, to=dim(all_sites)[1], by=1)){
  if (min(as.numeric(st_distance(all_sites$geometry[i],wild_sites$geometry)))<1.){
...
h0 = 14.43936 # use bandwidth selected by cross-validation
pdf('relative_risk_pos_vs_all_double.pdf')
...
cols = c("#FFFFFF",cols)
pal <- colorRampPalette(cols)
pdf('kde_positives_sample_level_double.pdf')
